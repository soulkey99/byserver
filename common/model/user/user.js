...
UserSecureSchema.plugin(BaseModel);

UserSchema.virtual('test_prop').get(function () {  //对默认头像进行的处理
    //do sth with the url
    return this.nick + ', abc, ' + this.intro;
});
...
UserSecureSchema.pre('save', function (next) {
...
mongoose.model('UserSecure', UserSecureSchema, 'userSecure');
