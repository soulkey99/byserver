...
module.exports = function (money_id, callback) {
    if (!callback) {
        callback = function () {

        }
    }
    proxy.Money.getMoneyInfoByID(money_id, (err, doc) => {
        if (err) {
        ...
        if (doc.status != 'pending') {//只要不是pending状态，就不再继续
            return callback(null, false);
        }
        console.log('doc status pending and channel: ' + doc.channel);
        if (doc.channel == 'alipay') { //ping++支付宝，继续获取charge状态
            pingpp.charges.retrieve(doc.charge.id, function (err, charge) {
                if (err) {
                    return callback(err, true);
                }
                // log.trace(`retrive charge: ${JSON.stringify(charge)}`);
                if (charge.paid) {//已支付，做相应操作(给教师加余额，给学生扣积分)
                    doPaid(doc, callback);
                } else {
                    callback(null, true);
                }
            });
        } else if (doc.channel == 'bc_alipay' || doc.channel == 'bc_wx') {
            API.getBills({
                channel: doc.channel == 'bc_alipay' ? 'ALI_APP' : 'WX_APP',
                timestamp: Date.now(),
                spay_result: true,
                bill_no: money_id
            }).then((response) => {
                console.log('api get bills, money_id: ' + money_id + ', resp: ' + JSON.stringify(response));
                if (response.result_msg == 'OK' && response.bills.length > 0) {
                    doPaid(doc, callback);
                } else {
                    callback(null, true);
                }
