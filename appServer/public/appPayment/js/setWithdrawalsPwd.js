...
function enterPwd(number){
    ...
    if(vm.pwd().length == 6){
        ...
        postObj.money = util.querystring("money")[0];
        postObj.passwd = util.sha256(vm.pwd().join(""));
        util.callServerFunction('withdraw&random='+parseInt(Math.random()*10000), postObj, function(data) {
            if(data.statusCode == 900) {
                if (u.indexOf('Android') > -1 || u.indexOf('Linux') > -1) {//安卓手机
                    javascript:callcall.jsCallback('withdrawsuccess', {});
                }
                window.location.href = "/appPayment/userBill.html?moneyId=" + data.money_id;
            } else if(data.statusCode == 898) {
                ...
                util.setSessionStorage("message",data.message);
