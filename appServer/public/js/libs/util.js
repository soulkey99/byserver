...
util.callServerFunction = function(funcName, postData, callback, method)
{
    ...
    $.ajax(
    {
        type : 'POST',
        async : true,
        url : 'https://callcall.soulkey99.com:9061/api?m=' + funcName,  //api?m=   http://test.soulkey99.com:8067/api?m=
        dataType : 'json',
        data : postData,
        /* 成功时直接返回数据 */
 ...
 * 异步调用服务器接口
 */
util.callServerFunctionV2 = function(funcName, postData, callback, method)
{
    var networkError =
    {
        "code" : -1000,
    ...
    $.ajax(
        {
            type : 'POST',
            async : true,
            url : 'https://callcall.soulkey99.com:9061/apiV2?m=' + funcName,  //api?m=   http://test.soulkey99.com:8067/api?m=
            dataType : 'json',
            data : postData,
            /* 成功时直接返回数据 */
            success : function(data, textStatus, jqXHR)
...
//错误返回代码提示
util.errorCodeApi = function(errorCode){
    switch(errorCode){
...
util.errorCodeAlert = function(data){
    alert("操作失败，错误代码：" + data.statusCode);
}
